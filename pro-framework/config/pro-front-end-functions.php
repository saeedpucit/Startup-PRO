 ...
 */
 ...
 if ( ! function_exists( 'startup_pro_header' ) ) {
     function startup_pro_header() {
         $startup_pro_options = startup_pro_get_theme_options();
         if ( $startup_pro_options['page-header-switch'] ) {
             get_template_part( 'pro-framework/headers/' . $startup_pro_options['header-layout'] );
             get_template_part( 'pro-framework/headers/mobile-header' );
             if ( $startup_pro_options['sticky-header'] && $startup_pro_options['header-layout'] != 'one-page-menu' ) {
...
if ( ! function_exists( 'startup_pro_rev_slider' ) ) {

	function startup_pro_rev_slider( $base, $position, $slider ) {
 ...
 */
 ...
 if ( ! function_exists( 'startup_pro_title_bar' ) ) {
		 function startup_pro_title_bar() {
...
if ( ! function_exists( 'startup_pro_loader' ) ) {

	function startup_pro_loader( ) {

		$startup_pro_options = startup_pro_get_theme_options();

		$output = $cont_bg = $loader_bg = '';

		if ( $startup_pro_options['load-bg'] ) {

			$cont_bg .= 'style="' . startup_pro_redux_bg_css( $startup_pro_options['load-bg'] ) . '"';

		}

		if ( isset( $startup_pro_options['loader-circle-color'] ) ) {

			$loader_bg = 'background-color:' . $startup_pro_options['loader-circle-color'] . ';';

		}

		if ( $startup_pro_options['loader-circle'] ) {

			$loader_bg .= startup_pro_redux_bg_css( $startup_pro_options['loader-circle'] );
...
if ( ! function_exists( 'startup_pro_output_logo' ) ) {

	function startup_pro_output_logo( $logo, $retina ) {
		...
		if ( empty($logo['url']) ) {
			...
			$output .= '<a href="' . esc_url( home_url( '/' ) ) . '"><img src="' . get_template_directory_uri() . '/assets/images/logo.png" alt="' . get_bloginfo( 'name' ) . '" class="normal"/><img style="width:110px;max-height:40px; height: auto !important" src="' . get_template_directory_uri() . '/images/logox2.png" alt="' . get_bloginfo( 'name' ) . '" class="retina"/></a>';
...
if ( ! function_exists( 'startup_pro_full_top_meta' ) ) {

	function startup_pro_full_top_meta( $post ) {
		...
		if ( ! is_search() ) {
			...
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				...
				comments_popup_link( esc_html__( '0', 'startup-pro' ), esc_html__( '1', 'startup-pro' ), esc_html__( '%', 'startup-pro' ) );
...
if ( ! function_exists( 'startup_pro_comments' ) ) {

	function startup_pro_comments() {
...
if ( ! function_exists( 'startup_pro_post_format' ) ) {

	function startup_pro_post_format() {
...
if ( ! function_exists( 'startup_pro_post_categories' ) ) {

	function startup_pro_post_categories($post_id) {
		...
		if ( $count >= 1 ) {
		  	...
		  	esc_html_e( 'Posted in:', 'startup-pro' );
...
if ( ! function_exists( 'startup_pro_page_featured_image' ) ) {

	function startup_pro_page_featured_image() {
		...
		$size = apply_filters( 'startup_pro_page_featured_image_size', 'full' );
...
if ( ! function_exists( 'startup_pro_post_tags' ) ) {

	function startup_pro_post_tags() {
...
if ( ! function_exists( 'startup_pro_grid_type_meta' ) ) {

	function startup_pro_grid_type_meta() {
...
if ( ! function_exists( 'startup_pro_medium_type_meta' ) ) {

	function startup_pro_medium_type_meta() {
		...
		echo '<span class="entry-author-link">' . esc_html__( 'By:', 'startup-pro' ) . '<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" rel="author">' . get_the_author() . '</a></span></span>';
...
if ( ! function_exists( 'startup_pro_posted_on' ) ) {

	function startup_pro_posted_on() {

		global $post;

		if ( is_sticky() && is_home() && ! is_paged() ) {

			echo '<span class="entry-featured">' . esc_html__( 'Sticky', 'startup-pro' ) . '</span>';
		...
		if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && startup_pro_categorized_blog() ) {
		...
		if ( ! is_search() ) {
			...
			if ( ! post_password_required() && ('open' == $post->comment_status) ) {
				...
				esc_html_e( 'Comments are Off', 'startup-pro' );
		...
		edit_post_link( esc_html__( 'Edit', 'startup-pro' ), '<span class="entry-edit-link">', '</span>' );
...
if ( ! function_exists( 'startup_pro_post_nav' ) ) {

	function startup_pro_post_nav() {
		...
		<nav class="post-navigation">
			...
			<h3 class="nav-previous"><?php previous_post_link( '%link', '<i class="fa fa-angle-left"></i> ' . esc_html__( 'Previous Post', 'startup-pro' ) . '<br /><strong><p>%title</p></strong>' ); ?></h3>

			<h3 class="nav-next"><?php next_post_link( '%link', esc_html__( 'Next Post', 'startup-pro' ) . ' <i class="fa fa-angle-right"></i><br /><strong><p>%title</p></strong>' ); ?></h3>
...
if ( ! function_exists( 'startup_pro_post_thumbnail' ) ) {

	function startup_pro_post_thumbnail( $link = '' ) {
		...
		global $startup_pro_blog_image_size;

		if ( is_search() ) {

			$startup_pro_blog_image_size = 'blog-small';

		}

		$size = ( empty( $startup_pro_blog_image_size ) ) ? $startup_pro_options['blog_image_size'] : $startup_pro_blog_image_size;

		$link = ( empty( $link ) ) ? esc_url(get_permalink()) : $link;

		if ( is_singular() ) {

			if ( $startup_pro_options['blog_featured_image'] ) {

				echo '<div class="entry-image">';

				$image_size = isset( $startup_pro_options['blog_single_image_size'] ) ? $startup_pro_options['blog_single_image_size'] : 'blog-small';
...
if ( ! function_exists( 'startup_pro_page_sidebar' ) ) {

	function startup_pro_page_sidebar( $base = 'right', $startup_pro_layout = 'right', $sidebar_name = 'sidebar-1' ) {

		if ( $base == $startup_pro_layout || $startup_pro_layout == 'both' ) {
...
if ( ! function_exists( 'startup_pro_get_button_class' ) ) {

	function startup_pro_get_button_class( $args = array() ) {
...
if ( ! function_exists( 'startup_pro_link_pages' ) ) {

	function startup_pro_link_pages( $args = '' ) {

		$defaults = array(

			'title'            => '<span class="pagination-title">' . esc_html__( 'Pages', 'startup-pro' ) . '</span>',
			...
			'nextpagelink'     => esc_html__( 'Next', 'startup-pro' ),

			'previouspagelink' => esc_html__( 'Previous', 'startup-pro' ),
...
if ( ! function_exists( 'startup_pro_paging_nav' ) ) {

	function startup_pro_paging_nav( $args = array() ) {
		...
		$template = $startup_pro_options['blog_layout'];

		$defaults = array(

			'nav'            => $startup_pro_options['blog_pagination'],

			'template'       => $startup_pro_options['blog_layout'],

			'posts_per_page' => get_option( 'posts_per_page' ),

			'size'           => $startup_pro_options['blog_image_size'],

			'columns'        => $startup_pro_options['blog_column'],
		...
		if ( $args['nav'] == 'load' ) {
			...
			$output .= '<a href="#" class="ajax-load-more ' . startup_pro_get_button_class( array( 'size' => 'xxs' ) ) . '" data-token="' . $uniqid . '">';

			$output .= esc_html__( 'Load More', 'startup-pro' );
			...
			wp_localize_script( 'pro-script', 'startup_pro_load_more_' . $uniqid, $args );
			...
			$links = paginate_links( array(
				...
				'prev_text' => esc_html__( 'Previous', 'startup-pro' ),

				'next_text' => esc_html__( 'Next', 'startup-pro' ),
...
if ( ! function_exists( 'startup_pro_portfolio_item' ) ) {

	function startup_pro_portfolio_item( $item_args ) {
		...
		$custom_layout = apply_filters( 'startup_pro_custom_portfolio_item', '', $item_args );
		...
		echo '<div class="isotope-item pro-fade ' . startup_pro_get_bootstrap( $columns ) . $item_class . '">';
...
if ( ! function_exists( 'startup_pro_story_item' ) ) {

	function startup_pro_story_item( $item_args ) {
		...
		$custom_layout = apply_filters( 'startup_pro_custom_story_item', '', $item_args );
...
if ( ! function_exists( 'startup_pro_event_item' ) ) {

	function startup_pro_event_item( $item_args ) {
		...
		$custom_layout = apply_filters( 'startup_pro_custom_event_item', '', $item_args );
...
if ( ! function_exists( 'startup_pro_comment_form' ) ) {

	function startup_pro_comment_form() {
 ...
 */
 if ( ! function_exists( 'startup_pro_footer_widgets' ) ) {
function startup_pro_footer_widgets() {
 ...
 if( !class_exists('redux') ) {
	 ...
	 return $output;
		 ...
		 $startup_pro_options = startup_pro_get_theme_options();
		 $output = '';
		 $widgets = $startup_pro_options['footer-widgets'];
...
if ( ! function_exists( 'netbee_footer_contact_details' ) ) {
	...
	function netbee_footer_contact_details() {
		...
		$contact_details = $startup_pro_options['contact-details'];
		...
		if ( $contact_details ) {
			...
			foreach ( $contact_details as $item ) {
				...
				$output .= '<div class="widget ' . startup_pro_get_bootstrap( $cols, 'md', true ) . '">';
...
if ( ! function_exists( 'startup_pro_background' ) ) {

	function startup_pro_background( $background, $parallax ) {
		...
		if ( $background['media'] && $parallax ) {
			...
			$css = startup_pro_redux_bg_css( $background );
...
function startup_pro_share_after_post( $content ) {

	$startup_pro_options = startup_pro_get_theme_options();

	if ( $startup_pro_options['enable-share'] && $startup_pro_options['addthis-html'] ) {
		...
		if ( is_single() ) {
			...
			$content .= $startup_pro_options['addthis-html'];
...
add_filter( "the_content", "startup_pro_share_after_post" );
...
if ( ! function_exists( 'startup_pro_comment' ) ) {

	function startup_pro_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) :
			...
			case 'trackback' :
				...
				<div class="pingback-content">
					...
					<span><?php esc_html_e( 'Pingback:', 'startup-pro' ); ?></span>

					<?php comment_author_link(); ?>

					<?php edit_comment_link( esc_html__( '(Edit)', 'startup-pro' ), ' ' ); ?>
				...
				<article id="div-comment-<?php comment_ID(); ?>">
					...
					<div class="comment-meta">
						...
						<time datetime="<?php comment_time( 'c' ); ?>">
							...
							printf( esc_html__( '%1$s at %2$s', 'startup-pro' ), get_comment_date(), get_comment_time() ); ?>
					...
					<?php if ( $comment->comment_approved == '0' ) : ?>
						...
						<em><?php esc_html_e( 'Your comment is awaiting moderation.', 'startup-pro' ); ?></em>
...
add_filter( 'comments_open', 'startup_pro_my_comments_open', 10, 2 );

function startup_pro_my_comments_open( $open, $post_id ) {

	$startup_pro_options = startup_pro_get_theme_options();

	if ( $startup_pro_options['allow-comments'] ) {
...
function startup_pro_remove_demo_mode() { // Be sure to rename this function to something more unique
...
add_action('init', 'startup_pro_remove_demo_mode');
