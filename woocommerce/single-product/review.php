		...
		<div class="comment-meta">
			...
			<?php if ( $rating && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) : ?>
				...
				<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="pro-star-rating"
				     title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'startup-pro' ), $rating ) ?>">
					<?php
					for ( $i = 0; $i < $rating; $i ++ ) {
			...
			<time itemprop="datePublished"
			      datetime="<?php echo get_comment_date( 'c' ); ?>"><?php echo get_comment_date( esc_html(__( '' , 'startup-pro' ) ) ); ?></time>
			<?php
			if ( get_option( 'woocommerce_review_rating_verification_label' ) === 'yes' ) {
				if ( wc_customer_bought_product( $comment->comment_author_email, $comment->user_id, $comment->comment_post_ID ) ) {
					echo '<em class="verified">(' . esc_html__( 'verified owner', 'startup-pro' ) . ')</em> ';
				}
			}
			?>
