 ...
 *
 */
if ( ! function_exists( 'startup_pro_get_bootstrap' ) ) {
	function startup_pro_get_bootstrap( $columns = 1, $device = 'md', $force = false ) {
		global $startup_pro_blog_column;
		$columns = ( ! empty( $startup_pro_blog_column ) && ! $force ) ? $startup_pro_blog_column : $columns;
		...
		$bootstrap_columns = apply_filters( 'startup_pro_get_bootstrap_columns', $bootstrap_columns );
 ...
 *
 */
if ( ! function_exists( 'startup_pro_get_bootstrap_col' ) ) {
	function startup_pro_get_bootstrap_col( $width = '' ) {
		$width = explode( '/', $width );
 ...
 *
 */
if ( ! function_exists( 'startup_pro_icon_class' ) ) {
	function startup_pro_icon_class( $icon, $before = false ) {
		if ( empty( $icon ) ) {
			return null;
		}
 ...
 *
 */
if ( ! function_exists( 'startup_pro_get_option' ) ) {
	function startup_pro_get_option( $name, $default = '' ) {
		$startup_pro_options = startup_pro_get_theme_options();
		$option = $default;
		if ( isset( $startup_pro_options[ $name ] ) && $startup_pro_options[ $name ] ) {
			$option = $startup_pro_options[ $name ];
		}
		return $option;
 ...
 *
 */
function startup_pro_get_menu_count( $depth = 0 ) {
	$startup_pro_menu_name = 'primary';
	$top_lvl = array();
	if ( ( $startup_pro_locations = get_nav_menu_locations() ) && isset( $startup_pro_locations[ $startup_pro_menu_name ] ) ) {
		$menu = wp_get_nav_menu_object( $startup_pro_locations[ $startup_pro_menu_name ] );
...
function after_item( $depth = 0 ) {
	$startup_pro_options = startup_pro_get_theme_options();
	$menu = startup_pro_get_menu_count();
	if ( isset( $startup_pro_options['logo_after'] ) ) {
		$after_id = $menu[ $startup_pro_options['logo_after'] + 1 ];
	} else {
		$count = count( $menu );
...
function get_header_layout() {
	...
	return $startup_pro_options['header-layout'];
}
/**
 *
...
function get_middle_logo() {
	...
	if ( $startup_pro_options['logo-upload']['url'] ) {
		$output .= '<a href="' . esc_url( home_url( '/' ) ) . '"><img src="' . $startup_pro_options['logo-upload']['url'] . '" alt="' . get_bloginfo( 'name' ) . '" class="normal"/>';
		if ( $startup_pro_options['logo-upload-retina'] ) {
			$output .= '<img style="width:' . $startup_pro_options['logo-upload']['width'] . 'px;max-height:' . $startup_pro_options['logo-upload']['height'] . 'px; height: auto !important" src="' . $startup_pro_options['logo-upload-retina']['url'] . '" alt="' . get_bloginfo( 'name' ) . '" class="retina"/>';
		};
		$output .= '</a>';
	} else {
		$output .= '<a href="' . esc_url( home_url( '/' ) ) . '"><img src="' . get_template_directory_uri() . '/assets/images/logo.png" alt="' . get_bloginfo( 'name' ) . '" class="normal"/><img style="width:110px;max-height:40px; height: auto !important" src="' . get_template_directory_uri() . '/images/logo@2x.png" alt="' . get_bloginfo( 'name' ) . '" class="retina"/></a>';
	}
	$output .= '</li>';
	return $output;
}
...
function normal_classes() {
	$startup_pro_options = startup_pro_get_theme_options();
	if ( $startup_pro_options['header-layout'] == 's3-header-v2' ) {
		return true;
	}
	return false;
}
...
function is_compact_menu() {
	$startup_pro_options = startup_pro_get_theme_options();
	if ( $startup_pro_options['header-layout'] == 's3-header-v1' || $startup_pro_options['header-layout'] == 's3-header-v2' || is_page( 1064 ) ) {
		return true;
	}
	return false;
}
 ...
 *
 */
if ( ! function_exists( 'startup_pro_css_compress' ) ) {
	function startup_pro_css_compress( $css ) {
 ...
 *
 */
if ( ! function_exists( 'startup_pro_set_wpautop' ) ) {
	function startup_pro_set_wpautop( $content, $force = true ) {
		if ( $force ) {
 ...
 *
 */
if ( ! function_exists( 'startup_pro_htmlentities' ) ) {
	function startup_pro_htmlentities( $text ) {
 ...
 *
 */
if ( ! function_exists( 'startup_pro_categorized_blog' ) ) {
	function startup_pro_categorized_blog() {
		if ( false === ( $all_the_cool_cats = get_transient( 'startup_pro_category_count' ) ) ) {
			$all_the_cool_cats = get_categories( array( 'hide_empty' => 1 ) );
			$all_the_cool_cats = count( $all_the_cool_cats );
			set_transient( 'startup_pro_category_count', $all_the_cool_cats );
		}
...
if ( ! function_exists( 'startup_pro_get_field' ) ) {
	function startup_pro_get_field( $field = array(), $value = '' ) {
		$output = "";
 ...
 *
 */
if ( ! function_exists( 'startup_pro_esc_string' ) ) {
	function startup_pro_esc_string( $num ) {
 ...
 *
 */
if ( ! function_exists( 'startup_pro_get_id_by_slug' ) ) {
	function startup_pro_get_id_by_slug( $slug ) {
 ...
 *
 */
if ( ! function_exists( 'startup_pro_redux_bg_css' ) ) {
	function startup_pro_redux_bg_css( $value = array() ) {
		$css = '';
